...
class PaymentController extends Controller
{
    ...
    private function initMomo(Transaction $transaction)
    {
        ...
        try {
            ...
            Log::error('MoMo payUrl not found in response', ['response' => $data]);
            throw new \Exception('MoMo không trả về payUrl.');

        } catch (\Exception $e) {
            Log::error('MoMo payment failed with circuit breaker', [
            ...
            return redirect()->route('payment.show', [
                ...
                'transaction' => $transaction->id,
            ])->with('warning', 'MoMo tạm thời không khả dụng. Vui lòng sử dụng phương thức thanh toán khác.');
        }
    }
