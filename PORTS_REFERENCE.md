...
- [Cách Kiểm Tra Ports](#cách-kiểm-tra-ports)
- [Xử Lý Port Conflicts](#xử-lý-port-conflicts)
...
**Kết luận:** ✅ **CÓ THỂ CHẠY SONG SONG** - Không có port conflicts!

---

## 🔍 CÁCH KIỂM TRA PORTS

### 1. Xem tất cả ports đang được Docker sử dụng
...
# Xem cả containers đã dừng
docker ps -a
```

### 2. Kiểm tra port cụ thể có đang dùng không (Windows)
...
**Nguyên nhân:** Port đã được sử dụng bởi container hoặc process khác

**Giải pháp:**

#### Option 1: Tìm và stop container đang dùng port

```bash
# Tìm container dùng port 8000
...
#### Option 2: Thay đổi port trong docker-compose

```yaml
# Đổi port host (bên trái)
ports:
  - "8001:8000"  # Host:Container
  # Thay vì "8000:8000"
```

#### Option 3: Kill process đang dùng port (Windows)

```bash
# Tìm PID đang dùng port
netstat -ano | findstr :8000

# Kill process (cần admin)
taskkill /PID <PID> /F
```
...
1. **Internal Communication:** Containers trong cùng network giao tiếp qua container name và internal port
   ```
   # Từ catalog_service → mysql_catalog
   DB_HOST=mysql_catalog
   DB_PORT=3306  # Internal port, không phải 3310
   ```

2. **External Access:** Truy cập từ host machine dùng `localhost` và external port
   ```
   # Từ máy tính → mysql_catalog
   mysql -h 127.0.0.1 -P 3310
   ```

3. **Health Checks:** Một số containers expose thêm ports cho health checks
   ```
   catalog_service:
...
4. **Security:** Trong production, nên:
   - Chỉ expose ports cần thiết
   - Dùng reverse proxy (Nginx)
   - Enable SSL/TLS
   - Restrict access với firewall
...
**📌 Cập nhật:** File này được tạo tự động. Nếu có thay đổi ports trong `docker-compose.yml` hoặc `docker-compose.microservices.yml`, hãy cập nhật file này.
