# 🏗️ Kiến Trúc Microservices - ElectroShop

## 📋 Tổng Quan

Hệ thống ElectroShop được thiết kế theo kiến trúc **Microservices**, trong đó mỗi chức năng được tách thành các service độc lập, giao tiếp với nhau qua API và Message Queue.
...
│    (Quản lý sản phẩm)     │ │   (Quản lý đơn hàng)      │ │     (Gửi thông báo)       │
│                           │ │                           │ │                           │
│  • Danh mục               │ │  • Tạo đơn hàng           │ │  • Gửi Email              │
│  • Sản phẩm               │ │  • Thanh toán             │ │  • SMS                    │
│  • Tìm kiếm               │ │  • Lịch sử mua hàng       │ │  • Push notification      │
│  • Đánh giá               │ │  • Giỏ hàng               │ │                           │
└───────────┬───────────────┘ └───────────┬───────────────┘ └───────────┬───────────────┘
            │                             │                             │
            ▼                             ▼                             ▼
    ┌───────────────┐             ┌───────────────┐             ┌───────────────┐
    │   MySQL DB    │             │   MySQL DB    │             │   Redis Queue │
...
## 🔄 Các Luồng Microservices Chi Tiết

### 1. 🛍️ Luồng Xem Sản Phẩm (Catalog Flow)
...
### 3. 🔔 Luồng Thông Báo (Notification Flow)
...
User         Order Service      Payment Gateway      Bank/VNPay      Notification
 ...
 │                │──────────────────────────────────────────────────────▶│
 │                │                    │                 │                │
 │ 10. Thank you  │                    │                 │    Email sent  │
 │◀───────────────│                    │                 │                │
```

---

## 📊 Mapping Services với Docker Containers
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                         electroshop_app (:8000)                              │
│                                                                              │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐       │
│   │   Catalog   │  │    Order    │  │    User     │  │   Admin     │       │
│   │   Module    │  │   Module    │  │   Module    │  │   Module    │       │
│   │             │  │             │  │             │  │             │       │
│   │ /products   │  │ /orders     │  │ /auth       │  │ /admin      │       │
│   │ /categories │  │ /cart       │  │ /profile    │  │ /dashboard  │       │
│   └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘       │
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────┐       │
│   │                    Shared Components                             │       │
│   │  • Models (Product, Order, User, Category, etc.)                │       │
│   │  • Middleware (Auth, CORS, Rate Limiting)                       │       │
│   │  • Events & Listeners (OrderCreated, UserRegistered)            │       │
│   │  • Jobs (SendEmail, ProcessPayment)                             │       │
│   └─────────────────────────────────────────────────────────────────┘       │
└─────────────────────────────────────────────────────────────────────────────┘
                │                                    │
                ▼                                    ▼
...
│   • products              │        │   • session:*             │
│   • categories            │        │   • queue:notifications   │
│   • orders                │        │   • queue:emails          │
│   • order_items           │        │                           │
│   • transactions          │        │                           │
│   • ratings               │        │                           │
...
### Asynchronous (Bất đồng bộ) - Message Queue
...
```php
// Events được publish
OrderCreated::class      // Khi tạo đơn hàng mới
OrderPaid::class         // Khi thanh toán thành công
OrderShipped::class      // Khi giao hàng
OrderDelivered::class    // Khi giao thành công
UserRegistered::class    // Khi đăng ký mới
PasswordReset::class     // Khi đặt lại mật khẩu
ProductViewed::class     // Khi xem sản phẩm
CartUpdated::class       // Khi cập nhật giỏ hàng
```
...
└─────────────────────────────────────────────────────────────────────────────┘
    ...
    │ (Send Email)  │ │(Update Stock) │ │ (Analytics)   │
    └───────────────┘ └───────────────┘ └───────────────┘
```

---

## 🛡️ Service Discovery & Health Check
