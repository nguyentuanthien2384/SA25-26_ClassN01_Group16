...
1. **Luôn dùng `-f docker-compose.microservices.yml`** khi chạy docker-compose để chỉ định đúng file.

2. **Dùng `--tail` khi xem logs** để tránh quá nhiều output.

3. **Backup trước khi dùng `down -v`** vì lệnh này sẽ xóa tất cả data.

4. **Sử dụng `docker stats`** để monitor resources và phát hiện memory leaks.

5. **Khi gặp lỗi**, luôn check logs trước: `docker logs --tail 100 <container>`

---

*Tài liệu được tạo: 28/01/2026*
*Dự án: ElectroShop Microservices*
